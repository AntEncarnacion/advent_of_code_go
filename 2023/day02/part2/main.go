...
func solver(cube_quantities map[string]int, game_text string) int {
	...
	for _, line := range strings.Split(game_text, "\n") {
		...
		result = result + (cube_count_map["red"] * cube_count_map["blue"] * cube_count_map["green"])
	}
	return result
}
...
func main() {
	...
	var game_text string = `Game 1: 1 green, 1 blue, 1 red; 3 green, 1 blue, 1 red; 4 green, 3 blue, 1 red; 4 green, 2 blue, 1 red; 3 blue, 3 green
	...
	fmt.Println(solver(cube_quantities, game_text))
}
